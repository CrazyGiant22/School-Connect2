...
$conn = $data;
$PASS_PERCENT = 45.0; // passing threshold

// -----------------------------------------------------------------------------
// Filters: academic year, term, course, lecturer
// -----------------------------------------------------------------------------
...
$selected_lecturer_id = isset($_GET['lecturer_id']) && $_GET['lecturer_id'] !== '' ? intval($_GET['lecturer_id']) : null;
...
// Lecturers list for filter
$lecturer_options = [];
$resLect = mysqli_query($conn, "SELECT id, username FROM user WHERE usertype='lecturer' ORDER BY username");
if ($resLect) {
    while ($rowL = mysqli_fetch_assoc($resLect)) {
        $lecturer_options[(int)$rowL['id']] = $rowL['username'];
    }
}

// Courses list for filter
$course_options = [];
$resCourses = mysqli_query($conn, "SELECT id, code, name FROM course ORDER BY code");
if ($resCourses) {
    while ($rowC = mysqli_fetch_assoc($resCourses)) {
        $course_options[(int)$rowC['id']] = $rowC['code'] . ' - ' . $rowC['name'];
    }
}

// -----------------------------------------------------------------------------
// Load CA data for analytics
// -----------------------------------------------------------------------------
...
if ($selected_year !== null) {
    // Prefer term-based year; assessments without term_id are still included but not filtered by year
...
if ($selected_lecturer_id !== null) {
    $lid = intval($selected_lecturer_id);
    $where[] = ' (c.lecturer_id = ' . $lid . ' OR lc.lecturer_id = ' . $lid . ')';
}
...
SELECT
  ...
  lect.username AS lecturer_name
FROM ca_scores cs
...
LEFT JOIN user lect ON lect.id = c.lecturer_id
...
// -----------------------------------------------------------------------------
// Compute aggregates using compute_ca_percent()
// -----------------------------------------------------------------------------

$courseStats = []; // course_id => stats
$global_total = 0;
$global_pass = 0;
...
$atRiskStudents = []; // list of [student_id, username, course_code, course_name, percent, letter]
...
// Sort at-risk students by percent ascending (worst first)
...
if ($export === 'courses') {
    ...
    header('Content-Disposition: attachment; filename="course_performance.csv"');
    $out = fopen('php://output', 'w');
    fputcsv($out, ['Course Code', 'Course Name', 'Lecturer', 'Students with CA', 'Average %', 'Pass Rate %']);

    foreach ($courseStats as $course_id => $st) {
        $meta = $courseMeta[$course_id] ?? ['code' => '', 'name' => '', 'lecturer_name' => ''];
...
?>
            ...
            <div class="page-subtitle">System-wide performance analytics based on Continuous Assessment (CA). Passing threshold: <?php echo $PASS_PERCENT; ?>%.</div>
                    ...
                    <form method="get" class="row g-3 align-items-end">
                        ...
                        <div class="col-md-3">
                            ...
                            <select name="lecturer_id" id="lecturer_id" class="form-select" onchange="this.form.submit()">
                                ...
                                <?php foreach ($lecturer_options as $lid => $lname): ?>
                                    <option value="<?php echo $lid; ?>" <?php if ($selected_lecturer_id === (int)$lid) echo 'selected'; ?>>
            ...
            <div class="grid-cols-4">
                <div class="stat-card">
                    ...
                    <div class="stat-card-content">
                        <h6>Student-Course Records</h6>
                        <h3><?php echo $global_total; ?></h3>
                    </div>
                </div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-card-content">
                        <h6>At-Risk Entries (&lt; <?php echo $PASS_PERCENT; ?>%)</h6>
            ...
            <div style="margin-top: 2rem;" class="d-flex justify-content-between align-items-center">
                ...
                <div>
                    <a href="admin_reports.php?<?php echo http_build_query(array_merge($_GET, ['export' => 'courses'])); ?>" class="btn btn-outline-secondary btn-sm">
                        Export Courses CSV
                    </a>
                </div>
            </div>
            ...
            <tbody>
                <?php if (empty($courseStats)): ?>
                    <tr><td colspan="5" class="text-center text-muted" style="padding: 1.5rem;">No CA records for the selected filters.</td></tr>
                <?php else: ?>
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($meta['lecturer_name'] ?? ''); ?></td>
            ...
            <div style="margin-top: 2rem;" class="d-flex justify-content-between align-items-center">
                ...
                <div>
                    <a href="admin_reports.php?<?php echo http_build_query(array_merge($_GET, ['export' => 'at_risk'])); ?>" class="btn btn-outline-secondary btn-sm">
                        Export At-Risk CSV
                    </a>
                </div>
            </div>
